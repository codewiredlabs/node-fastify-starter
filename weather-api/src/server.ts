...
const frontend = new nunjucks.Environment(new nunjucks.FileSystemLoader("src/web"));
...
function isEmptyQueryParams(obj: any) {
...
server.get("/", async (request, reply) => {
  ...
  console.log("CURRENT PARAMETERS IN REQUEST: ", queryParams.location);

  if (!isEmptyQueryParams(queryParams)) {

    if (queryParams.location === "") {
      const rendered = frontend.render("get_started.html", { environment, });
      await reply
    ...
    try {
      ...
      const rendered = frontend.render("weather.njk", {
        environment,
